...
const DataPreview = ({ sheetsData, handleDeleteRow, handleImport }) => {
  ...
  //     {sheetsData.length > 0 ? (
  //       <select
  //         className="mb-3 p-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white/10 backdrop-blur-lg text-white shadow-md transition-all"
  //         value={selectedSheet}
  //         onChange={(e) => {
  ...
  //       <table className="w-[70%] table-auto border-collapse border border-gray-400 shadow-lg rounded-lg mx-auto">
  //         <thead>
  ...
  //                 <td className="border px-4 py-2">
  //                   <button
  //                     className="text-white bg-red-500 hover:bg-red-600 px-3 py-1 rounded-md transition-all duration-200"
    ...
    <div className="mt-5">
      ...
      <div className="overflow-x-auto w-full">
        <table className="w-full sm:w-[70%] table-auto border-collapse border border-gray-400 shadow-lg rounded-lg mx-auto">
          <thead>
            <tr className="bg-white/10 backdrop-blur-lg text-left">
              <th className="border px-4 py-2 font-semibold min-w-[150px]">Name</th>
              <th className="border px-4 py-2 font-semibold min-w-[100px]">Amount</th>
              <th className="border px-4 py-2 font-semibold min-w-[150px]">Date</th>
              <th className="border px-4 py-2 font-semibold min-w-[100px]">Verified</th>
              <th className="border px-4 py-2 font-semibold min-w-[150px]">Actions</th>
            </tr>
          </thead>
          <tbody>
            {paginatedData.length > 0 ? (
              paginatedData.map((row, rowIndex) => (
                <tr key={rowIndex} className="hover:bg-white/10 backdrop-blur-lg">
                  <td className="border px-4 py-2">{row.name}</td>
                  <td className="border px-4 py-2">{formatNumber(row.amount)}</td>
                  <td className="border px-4 py-2">{format(new Date(row.date), "dd-MM-yyyy")}</td>
                  <td className="border px-4 py-2">{row.verified ? "Yes" : "No"}</td>
                  <td className="border px-4 py-2">
                    <button
                      className="text-white bg-red-500 hover:bg-red-600 px-3 py-1 rounded-md transition-all duration-200"
                      onClick={() => handleDelete(0, rowIndex)}
...
DataPreview.propTypes = {
  sheetsData: PropTypes.array.isRequired,
